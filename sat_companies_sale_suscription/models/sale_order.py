...
class SaleOrderTemplateInherit(models.Model):
    ...
    @api.onchange('sale_type_id','type_contract')
    def sale_order_template_domain(self):
        for record in self:
            record.sale_order_template_id = False
            if record.sale_type_id and record.type_contract:
                sales_orders = self.env['sale.order.template'].search([('sale_type_id','=', record.sale_type_id.id),('type_contract','=', record.type_contract)])
